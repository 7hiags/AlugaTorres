...
// inicializacao comum (sessão, db, notificações)
...
try {
    ...
    if ($emailExists && $isActive) {
        ...
        $subjectUser = 'Newsletter AlugaTorres Reativada';
        $messageUserHtml = '
            <p>A sua subscrição foi reativada com sucesso!</p>
            <p>Em breve receberá as nossas novidades e ofertas exclusivas sobre casas em Torres Novas.</p>
            <p>Pode gerir as suas preferências a qualquer momento.</p>';
        $bodyUser = EmailEstilizado('Subscrição Reativada', $messageUserHtml);
    ...
    // Enviar confirmação ao subscritor
    $subjectUser = 'Bem-vindo à Newsletter AlugaTorres';
    $messageUserHtml = '
            <p>Obrigado por se inscrever na nossa newsletter!</p>
            <p>Receberá em primeira mão:</p>
            <ul style="color: #555; padding-left: 20px;">
                <li>🏠 Novas casas disponíveis em Torres Novas</li>
                <li>💰 Ofertas exclusivas e promoções</li>
                <li>📅 Dicas para arrendatários e proprietários</li>
                <li>⭐ Avaliações de casas populares</li>
            </ul>
            <p>Pode cancelar a subscrição a qualquer momento.</p>';
    $bodyUser = EmailEstilizado('Newsletter - Subscrição Confirmada', $messageUserHtml);
    ...
    $messageAdminHtml = '<p>Novo subscritor registado: <strong>' . htmlspecialchars($email) . '</strong></p>';
    $bodyAdmin = EmailEstilizado('Novo Subscritor Newsletter', $messageAdminHtml);
