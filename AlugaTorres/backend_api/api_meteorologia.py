...
# Função para receber dados meteorológicos da API Open-Meteo
def get_weather_data():
    ...
    try:
        ...
        # 🌅 DETECÇÃO SOLAR PRECISA (sunrise/sunset Torres Novas)
        agora = datetime.now()
        daily = response.Daily()
        sunrise_time = pd.to_datetime(daily.Variables(4).ValuesAsNumpy()[0] + response.UtcOffsetSeconds(), unit='s', utc=True)
        sunset_time = pd.to_datetime(daily.Variables(5).ValuesAsNumpy()[0] + response.UtcOffsetSeconds(), unit='s', utc=True)
        
        is_dia = agora > sunrise_time and agora < sunset_time
        
        print(f"🌅 Solar: sunrise={sunrise_time.strftime('%H:%M')}, sunset={sunset_time.strftime('%H:%M')}, agora={agora.strftime('%H:%M')}, is_dia={is_dia}")
