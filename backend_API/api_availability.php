...
function blockDates($conn, $casa_id, $dates)
{
    try {
        foreach ($dates as $date) {
            ...
            ");
            $check_reserva->bind_param("iss", $casa_id, $date, $date);
            $check_reserva->execute();
            $check_reserva->store_result();

            if ($check_reserva->num_rows > 0) {
                // Data já está reservada, não pode bloquear
                echo json_encode(['error' => 'Não é possível bloquear a data ' . $date . ' porque já está reservada.']);
                return;
            }
