...
// AGORA sim verificar autenticação (exceto para calculate)
if (!isset($_SESSION['user_id']) && $action !== 'calculate') {
...
function createReservation()
{
    ...
    $stmt = $conn->prepare("SELECT id, proprietario_id, preco_noite, preco_limpeza, taxa_seguranca, titulo, capacidade FROM casas WHERE id = ? AND aprovado = 1");
    ...
    // Verificar se o número de hóspedes não excede a capacidade
    if ($hospedes > $casa['capacidade']) {
        echo json_encode(['error' => 'Número de hóspedes excede a capacidade máxima da casa (' . $casa['capacidade'] . ' pessoas)']);
        return;
    }
    ...
    // Criar reserva como pendente (aguarda confirmação do proprietário)
    ...
    $stmt->bind_param(
        ...
        $preco_info['noites'],
        $hospedes,
        $preco_info['preco_noite'],
        $preco_info['subtotal'],
        $preco_info['taxa_limpeza'],
        $preco_info['taxa_seguranca'],
        $preco_info['total']
    );
    ...
    try {
        ...
        $messageArrHtml = '
            <p>A sua reserva foi <strong>confirmada com sucesso</strong>!</p>
            <div style="background: #d4edda; padding: 15px; border-radius: 8px; border-left: 4px solid #28a745;">
                <h3 style="margin: 0 0 10px 0; color: #155724;">Detalhes da Reserva #' . $reserva_id . '</h3>
                <ul style="margin: 0; padding-left: 20px;">
                    <li><strong>Propriedade:</strong> ' . htmlspecialchars($tituloCasa) . '</li>
                    <li><strong>Check-in:</strong> ' . $checkin . '</li>
                    <li><strong>Check-out:</strong> ' . $checkout . '</li>
                    <li><strong>Hóspedes:</strong> ' . $hospedes . '</li>
                    <li><strong>Total:</strong> ' . number_format($preco_total, 2, ',', '.') . '€</li>
                </ul>
            </div>
            <p>Guarde este ID para referência futura: <strong>#' . $reserva_id . '</strong></p>
            <p>Precisa de alterar? Contacte-nos pelo formulário.</p>';
        $bodyArr = EmailEstilizado('Reserva Confirmada #' . $reserva_id, $messageArrHtml, $reserva_id, $arrendatario_nome);

        $subjectProp = "🔔 Nova Reserva Recebida: " . $tituloCasa;
        $messagePropHtml = '
            <p>Nova reserva criada para a sua propriedade <strong>' . htmlspecialchars($tituloCasa) . '</strong>.</p>
            <div style="background: #fff3cd; padding: 15px; border-radius: 8px; border-left: 4px solid #ffc107;">
                <h3 style="margin: 0 0 10px 0; color: #856404;">Detalhes da Reserva #' . $reserva_id . '</h3>
                <ul style="margin: 0; padding-left: 20px;">
                    <li><strong>Arrendatário:</strong> ' . htmlspecialchars($arrendatario_nome) . ' (' . htmlspecialchars($arrendatario_email ?? '') . ')</li>
                    <li><strong>Check-in:</strong> ' . $checkin . '</li>
                    <li><strong>Check-out:</strong> ' . $checkout . '</li>
                    <li><strong>Hóspedes:</strong> ' . $hospedes . '</li>
                    <li><strong>Total:</strong> ' . number_format($preco_total, 2, ',', '.') . '€</li>
            ...
            <p>A reserva está <strong>pendente de confirmação</strong>. Confirme ou rejeite no painel.</p>
            <p>ID da reserva: <strong>#' . $reserva_id . '</strong></p>';
        $bodyProp = EmailEstilizado('Nova Reserva #' . $reserva_id, $messagePropHtml, $reserva_id, $proprietario_nome);
...
function calculatePrice()
{
    ...
    error_log("🔍 CALCULATE PRICE - GET params: " . json_encode($_GET));
    
    $casa_id = isset($_GET['casa_id']) ? (int)$_GET['casa_id'] : 0;
    ...
    error_log("📊 CALCULATE PRICE - Params: casa_id=$casa_id, checkin=$checkin, checkout=$checkout, hospedes=$hospedes");
    ...
    // Validar formato datas YYYY-MM-DD
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $checkin)) {
        $error = "Formato de checkin inválido: $checkin";
        error_log("❌ ERRO: $error");
        echo json_encode(['error' => $error]);
        return;
    }
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $checkout)) {
        $error = "Formato de checkout inválido: $checkout";
        error_log("❌ ERRO: $error");
        echo json_encode(['error' => $error]);
        return;
    }

    // Verificar se checkin < checkout
    $checkin_date = new DateTime($checkin);
    $checkout_date = new DateTime($checkout);
    if ($checkin_date >= $checkout_date) {
        $error = 'Checkout deve ser após checkin';
        error_log("❌ ERRO: $error");
        echo json_encode(['error' => $error]);
        return;
    }
    ...
    if (isset($preco_info['error'])) {
        error_log("❌ ERRO no calculatePriceTotal: " . $preco_info['error']);
        echo json_encode(['error' => $preco_info['error']]);
        return;
    }
    
    error_log("✅ CALCULO SUCESSO: " . json_encode($preco_info));
...
function calculatePriceTotal($casa_id, $checkin, $checkout, $hospedes)
{
    ...
    error_log("🔢 calculatePriceTotal - Iniciando para casa_id: $casa_id");

    // Obter informações da casa
    $stmt = $conn->prepare("SELECT id, preco_noite, preco_limpeza, taxa_seguranca, capacidade FROM casas WHERE id = ?");
    $stmt->bind_param("i", $casa_id);
    $stmt->execute();
    $casa = $stmt->get_result()->fetch_assoc();

    if (!$casa) {
        $error = "Casa ID $casa_id não encontrada";
        error_log("❌ ERRO CASA: $error");
        return ['error' => $error];
    }

    // Validar capacidade
    if ($hospedes > $casa['capacidade']) {
        $error = "Hóspedes ($hospedes) excede capacidade ({$casa['capacidade']})";
        error_log("❌ ERRO CAPACIDADE: $error");
        return ['error' => $error];
    }
    ...
    try {
        ...
        error_log("📅 Noites calculadas: $noites");

        if ($noites <= 0) {
            return ['error' => 'Data de checkout deve ser após checkin'];
        }
    } catch (Exception $e) {
        return ['error' => 'Erro ao processar datas: ' . $e->getMessage()];
    }

    // CORREÇÃO: Buscar preços especiais com <=
    $precos_especiais = [];
    $stmt = $conn->prepare("SELECT data, preco FROM precos_especiais WHERE casa_id = ? AND data >= ? AND data <= ?");
    $stmt->bind_param("iss", $casa_id, $checkin, $checkout);
    $stmt->execute();
    $result = $stmt->get_result();
    
    while ($row = $result->fetch_assoc()) {
        $precos_especiais[$row['data']] = (float)$row['preco'];
    }
    
    error_log("💰 Preços especiais encontrados: " . count($precos_especiais));

    // Calcular preços
    $preco_noite_base = (float)$casa['preco_noite'];
    $subtotal = 0;
    $noites_especiais = 0;
    $precos_por_noite = [];
    ...
    for ($i = 0; $i < $noites; $i++) {
        ...
        $tem_especial = isset($precos_especiais[$date_str]);
        $preco_dia = $tem_especial ? $precos_especiais[$date_str] : $preco_noite_base;
        
        $subtotal += $preco_dia;
        if ($tem_especial) $noites_especiais++;
    ...
    // Taxas
    $taxa_limpeza = (float)($casa['preco_limpeza'] ?? 0);
    $taxa_seguranca = (float)($casa['taxa_seguranca'] ?? 0);

    return [
        'preco_noite' => $preco_noite_base,
        'noites' => $noites,
        'subtotal' => round($subtotal, 2),
        'taxa_limpeza' => round($taxa_limpeza, 2),
        'taxa_seguranca' => round($taxa_seguranca, 2),
        'total' => round($subtotal + $taxa_limpeza + $taxa_seguranca, 2),
        'precos_por_noite' => $precos_por_noite,
        'noites_especiais' => $noites_especiais
    ];
}
...
function listReservations()
{
    ...
    // Atualizar reservas confirmadas que já passaram do checkout para concluídas
    $hoje = date('Y-m-d');
    $stmt_update = $conn->prepare("
        ...
        AND r.data_checkout < ?
        AND (
            r.arrendatario_id = ? OR 
            (c.proprietario_id = ? AND ? = 'proprietario')
        )
    ");
    $stmt_update->bind_param("siis", $hoje, $user_id, $user_id, $user_type);
    $stmt_update->execute();
